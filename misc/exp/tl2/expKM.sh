...
regex='.*TIME=([0-9]+).*'
	   ...
	   do
			...
			let "c=$c+1"
			sleep 350 && ps aux | grep XXLLxxii$c | grep -v grep | awk '{print $2}'|xargs kill -9 &
                        result=$((../BaseBaseMarkSweep_x86_64-linux/rvm  -Dorg.deuce.transaction.contextClass=org.deuce.transaction.tl2mam2.Context -X:hydra:XXLLxxii$c -cp deucebench_stm.jar:deuceAgent-1.3.0.jar jstamp.KMeans.KMeans -m 40 -n 40 -t 0.005 -i jstamp/KMeans/inputs/random-n16384-d24-c16.txt -nthreads $j) 2>&1)
                        case $result in
                                *"Exceptidfgon"*) good=0;;
                                *) good=1;;
                        esac
	   ...
	   do
			...
			let "c=$c+1"
			sleep 350 && ps aux | grep XXLLxxii$c | grep -v grep | awk '{print $2}'|xargs kill -9 &
                        result=$((../BaseBaseMarkSweep_x86_64-linux/rvm  -Dorg.deuce.transaction.contextClass=org.deuce.transaction.tl2mam2.Context -X:hydra:XXLLxxii$c -cp deucebench_stm.jar:deuceAgent-1.3.0.jar jstamp.KMeans.KMeans -m 15 -n 15 -t 0.005 -i jstamp/KMeans/inputs/random-n16384-d24-c16.txt -nthreads $j) 2>&1)
                        case $result in
