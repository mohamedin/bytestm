...
regex='.*TIME=([0-9]+).*'
	   ...
	   do
			...
			let "c=$c+1"
			sleep 120 && ps aux | grep XXLLxxii$c | grep -v grep | awk '{print $2}'|xargs kill -9 &
                        result=$((../BaseBaseMarkSweep_x86_64-linux/rvm  -Dorg.deuce.transaction.contextClass=org.deuce.transaction.tl2.Context -Xms1024m -Xmx1024m -X:hydra:XXLLxxii$c -cp deucebench_stm.jar:deuceAgent-1.3.0.jar jstamp.Labyrinth3D.Labyrinth -i jstamp/Labyrinth3D/inputs/random-x32-y32-z3-n96.txt -t $j) 2>&1)
                        case $result in
                                *"Exception"*) good=0;;
