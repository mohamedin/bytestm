...
regex='.*TIME=([0-9]+).*'
	   ...
	   do
			...
			let "c=$c+1"
			sleep 120 && ps aux | grep XXLLxxii$c | grep -v grep | awk '{print $2}'|xargs kill -9 &
                        result=$((../BaseBaseMarkSweep_x86_64-linux/rvm  -Dorg.deuce.transaction.contextClass=org.deuce.transaction.tl2.Context -Xmx1024m -Xms1024m  -X:hydra:XXLLxxii$c -cp deucebench_stm.jar:deuceAgent-1.3.0.jar jstamp.vacation.Vacation -t 150000 -c $j -n 2 -q 90 -u 98 -r 16384) 2>&1)
                        case $result in
                                *"Exception"*) good=0;;
                                *) good=1;;
                        esac
	   ...
	   do
 			...
 			let "c=$c+1"
			sleep 120 && ps aux | grep XXLLxxii$c | grep -v grep | awk '{print $2}'|xargs kill -9 &
                       result=$((../BaseBaseMarkSweep_x86_64-linux/rvm  -Dorg.deuce.transaction.contextClass=org.deuce.transaction.tl2.Context -Xmx1024m -Xms1024m  -X:hydra:XXLLxxii$c -cp deucebench_stm.jar:deuceAgent-1.3.0.jar jstamp.vacation.Vacation  -t 150000 -c $j -n 4 -q 60 -u 90 -r 16384) 2>&1)
                        case $result in
                                *"Exception"*) good=0;;
                                *) good=1;;
                        esac
