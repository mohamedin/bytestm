...
public class Benchmark implements org.deuce.benchmark.Benchmark {
	...
	public void init(String[] args) {
		...
		if (args.length > 0) {
			...
			else if (args[0].equals("JavaHashMap"))
				m_set = new HashTableSet();
			else if (args[0].equals("LockFreeLL"))
				m_set = new IntSetLinkedListLockFree();
			else if (args[0].equals("LockFreeSL"))
				m_set = new SkipListLockFree();
			else if (args[0].equals("ConcRBTree"))
				m_set = new ConcurrentRBTree();
			else if (args[0].equals("SkipLock"))
				m_set = new IntSetSkipListLocks();
			else if (args[0].equals("LinkedLock"))
				m_set = new IntSetLinkedListLock();
			else
				error = true;
		} else
			error = true;
