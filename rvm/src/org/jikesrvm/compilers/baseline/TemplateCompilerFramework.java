 ...
 */
/*
 * This file is modified by Mohamed Mohamedin
 * STM support is added in memory load/store operations
 * Also, invoke static is modified so it can handle xBegin, xEnd
 */
...
public abstract class TemplateCompilerFramework
  ...
  private static MethodReference memLoadInt = MethodReference.findOrCreate(
		  TypeReference.findOrCreate(STM.class),
		  Atom.findOrCreateUnicodeAtom("memLoadInt"),
		  Atom.findOrCreateUnicodeAtom("(III)I"));
  private static MethodReference memLoadLong = MethodReference.findOrCreate(
		  TypeReference.findOrCreate(STM.class),
		  Atom.findOrCreateUnicodeAtom("memLoadLong"),
		  Atom.findOrCreateUnicodeAtom("(IJI)J"));
  
  private static MethodReference memLoadIntStatic = MethodReference.findOrCreate(
		  TypeReference.findOrCreate(STM.class),
		  Atom.findOrCreateUnicodeAtom("memLoadIntStatic"),
		  Atom.findOrCreateUnicodeAtom("(II)I"));
  private static MethodReference memLoadLongStatic = MethodReference.findOrCreate(
		  TypeReference.findOrCreate(STM.class),
		  Atom.findOrCreateUnicodeAtom("memLoadLongStatic"),
		  Atom.findOrCreateUnicodeAtom("(JI)J"));
  
  
  
  private static MethodReference memStoreInt = MethodReference.findOrCreate(
		  TypeReference.findOrCreate(STM.class),
		  Atom.findOrCreateUnicodeAtom("memStoreInt"),
		  Atom.findOrCreateUnicodeAtom("(IIIBZ)V"));
  private static MethodReference memStoreLong = MethodReference.findOrCreate(
		  TypeReference.findOrCreate(STM.class),
		  Atom.findOrCreateUnicodeAtom("memStoreLong"),
		  Atom.findOrCreateUnicodeAtom("(IJI)V"));
 
  private static MethodReference memStoreIntStatic = MethodReference.findOrCreate(
		  TypeReference.findOrCreate(STM.class),
		  Atom.findOrCreateUnicodeAtom("memStoreIntStatic"),
		  Atom.findOrCreateUnicodeAtom("(IIBZ)V"));
  private static MethodReference memStoreLongStatic = MethodReference.findOrCreate(
		  TypeReference.findOrCreate(STM.class),
		  Atom.findOrCreateUnicodeAtom("memStoreLongStatic"),
		  Atom.findOrCreateUnicodeAtom("(JI)V"));
  
  
  private static MethodReference memLoadIntArr = MethodReference.findOrCreate(
		  TypeReference.findOrCreate(STM.class),
		  Atom.findOrCreateUnicodeAtom("memLoadIntArr"),
		  Atom.findOrCreateUnicodeAtom("(III)I"));
  private static MethodReference memLoadLongArr = MethodReference.findOrCreate(
		  TypeReference.findOrCreate(STM.class),
		  Atom.findOrCreateUnicodeAtom("memLoadLongArr"),
		  Atom.findOrCreateUnicodeAtom("(IIJ)J"));
  private static MethodReference memLoadByteArr = MethodReference.findOrCreate(
		  TypeReference.findOrCreate(STM.class),
		  Atom.findOrCreateUnicodeAtom("memLoadByteArr"),
		  Atom.findOrCreateUnicodeAtom("(IIB)B"));
  private static MethodReference memLoadShortArr = MethodReference.findOrCreate(
		  TypeReference.findOrCreate(STM.class),
		  Atom.findOrCreateUnicodeAtom("memLoadShortArr"),
		  Atom.findOrCreateUnicodeAtom("(IIS)S"));
  
  
  private static MethodReference memStoreIntArr = MethodReference.findOrCreate(
		  TypeReference.findOrCreate(STM.class),
		  Atom.findOrCreateUnicodeAtom("memStoreIntArr"),
		  Atom.findOrCreateUnicodeAtom("(III)V"));
  private static MethodReference memStoreRefArr = MethodReference.findOrCreate(
		  TypeReference.findOrCreate(STM.class),
		  Atom.findOrCreateUnicodeAtom("memStoreRefArr"),
		  Atom.findOrCreateUnicodeAtom("(III)V"));
  private static MethodReference memStoreLongArr = MethodReference.findOrCreate(
		  TypeReference.findOrCreate(STM.class),
		  Atom.findOrCreateUnicodeAtom("memStoreLongArr"),
		  Atom.findOrCreateUnicodeAtom("(IIJ)V"));
  private static MethodReference memStoreByteArr = MethodReference.findOrCreate(
		  TypeReference.findOrCreate(STM.class),
		  Atom.findOrCreateUnicodeAtom("memStoreByteArr"),
		  Atom.findOrCreateUnicodeAtom("(IIB)V"));
  private static MethodReference memStoreShortArr = MethodReference.findOrCreate(
		  TypeReference.findOrCreate(STM.class),
		  Atom.findOrCreateUnicodeAtom("memStoreShortArr"),
		  Atom.findOrCreateUnicodeAtom("(IIS)V"));
  

  private static MethodReference setOrder = MethodReference.findOrCreate(
		  TypeReference.findOrCreate(STM.class),
		  Atom.findOrCreateUnicodeAtom("setOrder"),
		  Atom.findOrCreateUnicodeAtom("(J)V"));
  
  private static MethodReference xBegin = MethodReference.findOrCreate(
		  TypeReference.findOrCreate(STM.class),
		  Atom.findOrCreateUnicodeAtom("xBegin"),
		  Atom.findOrCreateUnicodeAtom("()V"));

  private static MethodReference xCommit = MethodReference.findOrCreate(
		  TypeReference.findOrCreate(STM.class),
		  Atom.findOrCreateUnicodeAtom("xCommit"),
		  Atom.findOrCreateUnicodeAtom("()I"));
  ...
  protected final MachineCode genCode() {
    ...
    STM.enableSTM = BBProfiler.profile(method);
    ...
    while (bcodes.hasMoreBytecodes()) {
      ...
      switch (code) {
        ...
        case JBC_iaload: {
          ...
          if (STM.enableSTM) emit_dup2();
          emit_iaload();
          if (STM.enableSTM) emit_resolved_invokestatic(memLoadIntArr);
          break;
        }
        ...
        case JBC_laload: {
          ...
          if (STM.enableSTM) emit_dup2();
          emit_laload();
          if (STM.enableSTM) emit_resolved_invokestatic(memLoadLongArr);
          break;
        }
        ...
        case JBC_baload: {
          ...
          if (STM.enableSTM) emit_dup2();
          emit_baload();
          if (STM.enableSTM) emit_resolved_invokestatic(memLoadByteArr);
          break;
        }
        ...
        case JBC_caload: {
          ...
          if (STM.enableSTM) emit_dup2();
          emit_caload();
          if (STM.enableSTM) emit_resolved_invokestatic(memLoadShortArr);
          break;
        }
        ...
        case JBC_iastore: {
          ...
          if (STM.enableSTM) 
        	  emit_resolved_invokestatic(memStoreIntArr);
          else
        	  emit_iastore();
        ...
        case JBC_lastore: {
          ...
          if (STM.enableSTM) 
        	  emit_resolved_invokestatic(memStoreLongArr);
          else
        	  emit_lastore();
          break;
        }
        ...
        case JBC_aastore: {
          ...
          if (STM.enableSTM) 
        	  emit_resolved_invokestatic(memStoreRefArr);
          else
        	  emit_aastore();
          break;
        }
        ...
        case JBC_bastore: {
          ...
          if (STM.enableSTM) 
        	  emit_resolved_invokestatic(memStoreByteArr);
          else
        	  emit_bastore();
          break;
        }
        ...
        case JBC_castore: {
          ...
          if (STM.enableSTM) 
        	  emit_resolved_invokestatic(memStoreShortArr);
          else
        	  emit_castore();
          break;
        }
        ...
        case JBC_getstatic: {
          ...
          if (STM.enableSTM){
        	  if(dynamic){
        		  BaselineCompilerImpl.emitDynamicLinkingSequence(asm, GPR.EAX, fieldRef, true);
        		  asm.emitPUSH_Reg(GPR.EAX);
        	  }
        	  else{
        	  ...
        	  if(fieldRef.getSize() <= BYTES_IN_INT)
        		  emit_resolved_invokestatic(memLoadIntStatic);
        	  else
        		  emit_resolved_invokestatic(memLoadLongStatic);
          }
        ...
        case JBC_putstatic: {
          ...
          if (STM.enableSTM){
        	  ...
        	  if(fieldRef.getSize() <= BYTES_IN_INT){
        		  asm.emitPUSH_Imm(fieldRef.getSize());
        		  asm.emitPUSH_Imm(fieldRef.getFieldContentsType().isReferenceType()? 1:0);
        		  emit_resolved_invokestatic(memStoreIntStatic);
        	  }
        	  else
        		  emit_resolved_invokestatic(memStoreLongStatic);
          }
          else
          {
        ...
        case JBC_getfield: {
          ...
          boolean dynamic = fieldRef.needsDynamicLink(method);
          if (dynamic) {
              // Forbidden from uninterruptible code as dynamic linking can cause
              // interruptions
          ...
          if (STM.enableSTM){
        	  ...
        	  if(fieldType.isLongType() || fieldType.isDoubleType() || (VM.BuildFor64Addr && fieldType.isWordType()))
        		  emit_resolved_invokestatic(memLoadLong);
        	  else
        		  emit_resolved_invokestatic(memLoadInt);
          }
        ...
        case JBC_putfield: {
          ...
          if (STM.enableSTM){
        	  ...
        	  TypeReference fieldType = fieldRef.getFieldContentsType();
        	  if(fieldType.isLongType() || fieldType.isDoubleType() || (VM.BuildFor64Addr && fieldType.isWordType()))
        		  emit_resolved_invokestatic(memStoreLong);
        	  else{
        		  asm.emitPUSH_Imm(fieldRef.getSize());
        		  asm.emitPUSH_Imm(fieldRef.getFieldContentsType().isReferenceType()? 1:0);
        		  emit_resolved_invokestatic(memStoreInt);
        	  }
        ...
        case JBC_invokestatic: {
	          ...
	          MethodReference methodRef = bcodes.getMethodReference();
	          if (STM.enableSTM && methodRef.getType().getName().toString().equals("Lstm/STM;") && methodRef.getName().toString().equals("setOrder")){
	          	  emit_dup2();
	          }
	          ...
	          if(STM.enableSTM){
	          	  if (methodRef.getType().getName().toString().equals("Lstm/STM;"))
	          		  if (methodRef.getName().toString().equals("xBegin")){
	          			  emit_resolved_invokestatic(xBegin);
	          			  xBeginIndex = biStart;//retry from the instruction after xBegin
	          		  }
	          		  else if (methodRef.getName().toString().equals("xCommit")){
	          			  emit_resolved_invokestatic(xCommit);
	          			  emit_ifne(xBeginIndex);
	          		  }
	          		  else if (methodRef.getName().toString().equals("setOrder")){
	          			  emit_resolved_invokestatic(setOrder);
	          		  }
	          }
          break;
        }
