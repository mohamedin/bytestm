...
final class BuildReferenceMaps implements BytecodeConstants, ClassLoaderConstants, BBConstants {
  ...
  public void buildReferenceMaps(NormalMethod method, int[] stackHeights, byte[] localTypes,
    ...
    if (debug) 
    	VM.sysWrite("getLocalWords() : " + method.getLocalWords() + "\n");
...
//    VM.sysWriteln(method.getName() + " " + method.getDescriptor() + "(" + method.getOperandWords() + " " + method.getLocalWords() + ")");
...
//      VM.sysWriteln(method.getName() + " Processing block number is  " + basicBlocks[currBBNum].getBlockNumber() + " " + start + "-" + end + " bc=" + bcodes.length());
      ...
      if (debug){
	      VM.sysWrite("BBMAP:\n");
	      for(int i=0; i<bbMaps.length; i++){
	    	  VM.sysWrite(" " + i + " ");
	    	  if(bbMaps[i]!=null)
		    	  for (int j = 0; j < bbMaps[i].length; j++) {
		    		  VM.sysWrite(bbMaps[i][j]);
		    	  }
	    	  VM.sysWrite("\n");
	      }
      }
      ...
      while (bcodes.index() <= end) {
        ...
        if (debug){
          VM.sysWrite("opcode : " + opcode + " "+ currBBStkTop + " " + biStart + "\n");
          VM.sysWrite("current map: ");
          for (int j = 0; j <= currBBStkTop; j++) {
            VM.sysWrite(currBBMap[j]);
          }
          VM.sysWrite(" (" + currBBMap.length + "\n");
        }
