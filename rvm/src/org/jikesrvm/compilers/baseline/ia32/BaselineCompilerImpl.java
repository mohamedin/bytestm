...
public abstract class BaselineCompilerImpl extends BaselineCompiler implements BaselineConstants, SizeConstants {
  ...
  protected final void emit_iload(int index) {
    Offset offset = localOffset(index, false);
  ...
  protected final void emit_istore(int index) {
    Offset offset = localOffset(index, true).minus(WORDSIZE); // pop computes EA after ESP has moved by WORDSIZE!
    if (offset.EQ(Offset.zero())) {
  ...
  protected final void emit_lstore(int index) {
    if (VM.BuildFor32Addr) {
      if (SSE2_BASE) {
        Offset offset = localOffset(index, true).minus(WORDSIZE);
        ...
        Offset offset = localOffset(index + 1, true).minus(WORDSIZE);
  ...
  protected final void emit_iinc(int index, int val) {
    Offset offset = localOffset(index, true);
  ...
  protected final void emit_ret(int index) {
    Offset offset = localOffset(index, true);	//TODO check ret
    // Can be:
  ...
  private void genMonitorEnter() {
    if (method.isStatic()) {
      ...
      // push "this" object
      asm.emitPUSH_RegDisp(ESP, localOffset(0, true));	//TODO
    }
    // pass 1 parameter
  ...
  private void genMonitorExit() {
    if (method.isStatic()) {
      ...
      asm.emitPUSH_RegDisp(ESP, localOffset(0, true));                    // push "this" object
    }
   ...
   */
  private Offset localOffset(int local, boolean isWrite) {
   ...
   */
  public static void emitDynamicLinkingSequence(Assembler asm, GPR reg, MemberReference ref, boolean couldBeZero) {
