...
public abstract class RVMMethod extends RVMMember implements BytecodeConstants {
   ...
   */
  public static RVMMethod createDefaultConstructor(TypeReference klass, MemberReference memRef) {
  ...
  RVMMethod createReflectionMethod(TypeReference reflectionClass, int[] constantPool,
    ...
    if (!isStatic()) {
      ...
      bytecodes[curBC] = JBC_aload_1;	// load first argument (Object of called method)
      curBC++;
    } else {
      // static call
      bytecodes = new byte[8 * numParams + 7];
    }
    for (int i=0; i < numParams; i++) {	// start of argument loop retrieving 
      ...
      bytecodes[curBC] = (byte)JBC_aload_2;		// load second argument (Array of original method parameters)
      bytecodes[curBC+1] = (byte)JBC_sipush;	// push the index of the parameter
      bytecodes[curBC+2] = (byte)(i >>> 8);
      bytecodes[curBC+3] = (byte)i;
      bytecodes[curBC+4] = (byte)JBC_aaload;	// load Object array with index i
      if (!parameters[i].isPrimitiveType()) {
        bytecodes[curBC+5] = (byte)JBC_checkcast;	// Cast object array element to the desiered type
        if (VM.VerifyAssertions) VM._assert(parameters[i].getId() != 0);
        constantPool[i+1] = ClassFileReader.packCPEntry(CP_CLASS, parameters[i].getId());	// add entry with the class type (java.lang.Integer) to constant pool
        bytecodes[curBC+6] = (byte)((i+1) >>> 8);	// index of constant pool newly created index 
        ...
        bytecodes[curBC+5] =
          ...
          bytecodes[curBC+7] =
            (byte)JBC_nop;
      } else {	// convert primitive types (e.g. int, char, boolean, ...) from Object back to data types using ready-made static methods at class RefelectionBase
        ...
        constantPool[i+1] = ClassFileReader.packCPEntry(CP_MEMBER, unboxMethod.getId());	// add entry with the resolved method name to constant pool
        bytecodes[curBC+6] = (byte)((i+1) >>> 8);	// index of constant pool newly created index 
        bytecodes[curBC+7] = (byte)(i+1);
      }
      curBC+=8;
    }	// end of argument loop retrieving 
    if (isStatic()) {	// generate calling instruction for the method
    ...
    constantPool[numParams+1] = ClassFileReader.packCPEntry(CP_MEMBER, getId());	// add entry with the current method name to constant pool
    bytecodes[curBC+1] = (byte)((numParams+1) >>> 8);
    bytecodes[curBC+2] = (byte)(numParams+1);
    if (interfaceCall) {
      // invokeinterface bytecodes are historically longer than others :) hehehehehe
      curBC+=2;
    }
    ...
    if (!returnType.isPrimitiveType() || returnType.isWordType()) {
      ...
      bytecodes[curBC+4] = (byte)JBC_nop;
      bytecodes[curBC+5] = (byte)JBC_nop;
    } else {	// convert returned primitive types (e.g. int, char, boolean, ...) back to Object using ready-made static methods at class RefelectionBase
      ...
      constantPool[numParams+2] = ClassFileReader.packCPEntry(CP_MEMBER, boxMethod.getId());	// add entry with the resolved method name to constant pool
