...
public final class RVMClass extends RVMType implements Constants, ClassLoaderConstants {
  ...
  public int addSyntheticMethod(RVMMethod syntheticMethod) {
	  // modify declared methods array
	  RVMMethod[] newDeclaredMethods = new RVMMethod[declaredMethods.length+1];
	  System.arraycopy(declaredMethods, 0, newDeclaredMethods, 0, declaredMethods.length);
	  newDeclaredMethods[declaredMethods.length] = syntheticMethod;
	  declaredMethods = newDeclaredMethods;
	  // modify the constants pool
	  int[] newConstantPool = new int[constantPool.length+1];
	  System.arraycopy(constantPool, 0, newConstantPool, 0, constantPool.length);
	  int constantPoolIndex = constantPool.length;
	  newConstantPool[constantPoolIndex] = ClassFileReader.packCPEntry(CP_MEMBER, syntheticMethod.getId());	// add entry with the current method name to constant pool
  ...
  public void extendConstantsPool(int[] extension) {
	  // modify the constants pool
	  int[] newConstantPool = new int[constantPool.length+extension.length];
	  System.arraycopy(constantPool, 0, newConstantPool, 0, constantPool.length);
	  System.arraycopy(extension, 0, newConstantPool, constantPool.length, extension.length);
	  constantPool = newConstantPool;
  }
  ...
  /**
   * Retrieve constants pool array
   */
  public int[] getConstantsPool(){
	  return constantPool;
  }
   ...
   */
  public RVMClass(TypeReference typeRef, int[] constantPool, short modifiers, RVMClass superClass,
